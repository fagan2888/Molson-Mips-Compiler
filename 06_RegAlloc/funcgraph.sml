...
struct
	...
	fun compare({from=f1,to=t1},{from=f2,to=t2}) = case Key.compare(f1,f2) of EQUAL => Key.compare(t1,t2) | x => x
end
...
fun getNode(g,nid) = case NodeMap.find(g,nid) of NONE => raise NoSuchNode(nid) | SOME x=> x
fun addNode(g,nid,d) = NodeMap.insert(g,nid,(nid,d,NodeSet.empty,NodeSet.empty))
fun addNode'(g,nid,d) =
  let
		val n = (nid,d,NodeSet.empty,NodeSet.empty)
...
fun changeNodeData(g,nid,d) =
  let
		val (_,_,s,p) = getNode(g,nid)
  in
		NodeMap.insert(g,nid,(nid,d,s,p))
  end

fun adjustSets(g,{from,to},f) = case Key.compare(from,to) of
	EQUAL =>
		let
			val (i,d,s,p) = getNode(g,from)
		in
		  NodeMap.insert(g,from,(i,d,f(s,from),f(p,from)))
		end
  | _ =>
		let
			val (fi,fd,fs,fp) = getNode(g,from)
		 	val (ti,td,ts,tp) = getNode(g,to)
		 	val fs' = f(fs,to)
		 	val tp' = f(tp,from)
	  in
		 	NodeMap.insert(NodeMap.insert(g,from,(fi,fd,fs',fp)), to, (ti,td,ts,tp'))
	  end

fun addEdge(g,{from,to}) = adjustSets(g,{from=from,to=to},NodeSet.add)
fun removeEdge(g,{from,to}) = adjustSets(g,{from=from,to=to},NodeSet.delete) handle NotFound => raise NoSuchEdge(from,to)
fun removeEdge'(g,{from,to}) = adjustSets(g,{from=from,to=to},NodeSet.delete) handle NotFound => g
...
fun removeNode(g,nid) =
  let
		val (_,_,succ,pred) = getNode(g,nid)
		val es1 = NodeSet.foldl (fn(s,es)=>EdgeSet.add(es,{from=nid,to=s})) EdgeSet.empty succ
		val es2 = NodeSet.foldl (fn(p,es)=>EdgeSet.add(es,{from=p,to=nid})) es1 pred
...
fun isAdjacent ((n1,_,s1,p1),(n2,_,s2,p2)) = NodeSet.member(NodeSet.union(s1,p1),n2) orelse NodeSet.member(NodeSet.union(s2,p2),n1)
