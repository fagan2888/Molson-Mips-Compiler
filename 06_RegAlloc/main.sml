...
struct
  ...
  fun generate filename venv ir =
    let
      ...
      fun printstring (T.STRING(label, string)) = output (label ^ ":\n" ^ ".asciiz \"" ^ string ^"\"\n")
        | printstring _ = ()

      fun printinstrs format instr = output (format instr)
      ...
      val (allocation, spills) = Color.color {interference=igraph, initial=Registers.tempMap, registers=Registers.registers}
      val format = C.format(fn (temp) => case Temp.Map.find(allocation,temp) of SOME str => ("$" ^ str) | NONE => ("$ERROR"))
      val _ = if List.length(spills)>0 then Error.msg 0 ("register allocation spill") else ()

      val _ = output ".text\n"
      val _ = map (printinstrs format) instrs
      val _ = output ".data\n"
