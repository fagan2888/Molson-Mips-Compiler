    ...
    let
      ...
      fun checkBody (body,res,pos,level,label,temps,venv) =
        let
          ...
          val body = T.SEQ[T.MOVE (T.TEMP R.RV,T.unEx(#exp b)), T.MOVE(T.TEMP R.SP, T.BINOP(T.PLUS, T.TEMP R.SP, T.CONST (!T.tuplesp)))]
          val _ = T.frags := T.getResult() @ [T.PROC{body=body,name=label,formals=temps}]
        in
          venv
        end
